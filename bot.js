...
var bot_token = '5882994498:AAEvhireRzHCTN6P6LfN5rsMV0WONqNJDYA'; //YOUR BOT TOKEN HERE
var bot_name = 'Nodejs Test Subject'; // Bot Name
...
const admin_id = 1868885213;
const admin_id2 = 6074077795;
var mongo_url = 'mongodb+srv://Shivanshmundra:<password>@cluster0.sdnkism.mongodb.net/test'; // Your Mongo URL Here
const buttonsLimit = {
    window: 1000,
    limit: 1,
...
bot.hears(/^\/start (.+[1-9]$)/, async (ctx) => {
    try {
        ...
        if (bots == 'Active') {
            ...
            bot.telegram.sendMessage(ctx.from.id,"*©Share Your Contact In Order To Start Using The Bot. This Is Just A Phone Number Verification\n\n⚠️Note : We Will Not Share Your Details With Anyone* @Advancdcoder",{parse_mode:"markdown",reply_markup:{keyboard: [[{text:"💢 Share Contact",request_contact:true}]],resize_keyboard: true}})
        } else {
...
bot.hears('🗂 Wallet', async (ctx) => {
    try {
        ...
        if (bots == 'Active') {
            ...
            if (flag == channel.length) {
                ctx.reply(
            '*✏️ Now Send Your ' + currency + ' Wallet Address To Use It For Future Withdrawals*\n\n⚠️ _This Wallet Will Be Used For Future Withdrawals @Beastcampaign!!_', { parse_mode: 'markdown', reply_markup: { keyboard: [['⛔ Cancel']], resize_keyboard: true } }
                )
